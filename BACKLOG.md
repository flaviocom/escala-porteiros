...
| P8.4 | ⚪ **Horário real (regra máxima do dono) implementado no `escala-geral`**, coexistindo com o período — só motor de teste, produção não tocada. 4ª auditoria achou 4 defeitos reais (vira-a-noite colapsava o bloqueio em silêncio; a tela aceitava a entrada que causava isso; a régua "independente" tinha critério mais frouxo que a régua principal; um rótulo cravado sobrou numa tela) — todos corrigidos com teste de regressão | [DB-067](DIARIO_DE_BORDO.md), S-070 | ✅ 20/08 — nada mudou aqui. 5ª auditoria (verificação cética das correções) em andamento no `escala-geral`; decisão 👤 futura de portar para produção segue separada, item a item |
