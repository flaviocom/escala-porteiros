...
| P4.17 | 🔴 **O script escrevia a escala por um caminho PIOR que o da tela.** `gerar-bloco.mjs` usava `gerar()` guloso; a área administrativa usa `gerarVariasVersoes()` (8 versões, a melhor por piso e depois Jain). Fonte dupla silenciosa: as duas passam nas 16 regras, mas uma distribui melhor | `scripts/gerar-bloco.mjs` | ✅ **FECHADO 05/08** — mesmo caminho da tela, e o log imprime as 8 versões comparadas |
...
| P5.1 | 🔴 **Dados pessoais em repositório PÚBLICO, com histórico imutável.** 16 nomes reais de voluntários ficam em `pessoas.json` e em cada turno, para sempre — e o projeto promete que o passado **não se reescreve**. Achado do teste de portabilidade: *"zero linhas sobre isso em 30 documentos, num produto com intenção de comercialização"* | `public/dados/pessoas.json` · repositório público | 🔴 **DECISÃO DO FLAVIO** — qual a base legal, e como alguém exerce direito de exclusão num histórico git público? Saídas possíveis: repositório privado + Pages pago · só primeiro nome · identificadores no dado e nomes numa camada separada |
...
| P5.5 | 🟠 **Idioma cravado.** É a única coisa que varia de cliente e **não** virou dado, contrariando §0. O produto atende só o Brasil, ou é dívida? | todo texto de tela | 🔴 **DECISÃO DO FLAVIO** |
