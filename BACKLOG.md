...
> [`ESTADO.md`](ESTADO.md) → [`handoff mais recente`](docs/handoff/HANDOFF_2026-08-08-b.md) → **`BACKLOG.md` (você está aqui)**
>
...
| P5.3 | 🟠 **`blocos.json` só cresce.** Nunca há arquivamento; a Contents API tem teto por arquivo | `public/dados/blocos.json` | Declarado. **Medido em 08/08/2026: 41.815 bytes, 183 turnos → 228 bytes/turno, ~49 KB/ano.** Qualquer teto em megabytes dá décadas de folga. O número exato do teto segue POR MEDIR — a rede do contêiner não alcança a documentação da API, e teto sem fonte seria chute (fica para uma máquina com rede) |
