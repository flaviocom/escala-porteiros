...
| P8.4 | ⚪ **Horário real (regra máxima do dono) implementado no `escala-geral`**, coexistindo com o período — só motor de teste, produção não tocada. 4ª auditoria achou 4 defeitos reais; 5ª auditoria achou uma REGRESSÃO na própria correção do vira-a-noite (início-igual-a-fim bloqueava o dia inteiro, por uma segunda porta de entrada sem validação) mais uma lacuna de paridade — todos corrigidos, a última vez na origem (`malha.ts`), não só na tela | [DB-067](DIARIO_DE_BORDO.md), S-070 | ✅ 20/08 — nada mudou aqui. 6ª auditoria (verificação cética) em andamento no `escala-geral`; decisão 👤 futura de portar para produção segue separada, item a item |
