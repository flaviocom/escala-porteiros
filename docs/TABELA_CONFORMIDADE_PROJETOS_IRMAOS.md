...
| Ordem de leitura declarada | **DUAS versões conflitantes:** `AGENTS.md:7-8` diz AGENTS→`docs/AMBIENTE_E_ACESSOS.md`→ESTADO→BACKLOG; `CLAUDE.md:10-13` diz ESTADO→Handoff→BACKLOG (nunca cita AGENTS.md) | **Uma única versão:** `AGENTS.md:9-11` diz AGENTS→ESTADO→handoff mais recente→BACKLOG |
| Seção "Mesa do dono" em `ESTADO.md` | ✅ `ESTADO.md:8-16` — criada nesta frente, antes vazia de menção | ✅ `ESTADO.md:18-26` — criada nesta frente |
| Seção MÉTODO em `AGENTS.md` | ✅ já existia antes desta frente — `AGENTS.md:57-59`, aponta para `docs/METODO_GAUNTLET_CHARMWAY.md`, instituída 13/08/2026 | ✅ criada nesta frente — `AGENTS.md:73-77`, aponta para `METODO_GAUNTLET_LOOP.md` (global) + `ENGINEERING_LOOP.md` |
...
| Deploy | Vercel — `.vercel/project.json` presente (`test -f`) + `docs/pre-voo.json:43` | GitHub Pages — branch `main` + `/docs` (`docs/pre-voo.json:37`) |
| Site | <https://charmway.net> — `AGENTS.md:27` | <https://flaviocom.github.io/escala-porteiros/> — `AGENTS.md:38` |
...
`credenciais_exigidas`/`conectores_mcp`) **não é o que `scripts/pre-voo.mjs:82-99` lê** — a função
...
| Bloco `_metadata_projeto` | acrescentado nesta frente — `docs/pre-voo.json:39-44` | acrescentado nesta frente — `docs/pre-voo.json:33-38` |
| Documentos acima do teto (400 linhas / 40 KB) | 🔴 **17**, com isenção declarada (`docs.bloqueia:false`, motivo em `docs/pre-voo.json:27`) — specs congeladas classificadas no regime errado, já rastreado no `BACKLOG.md` do projeto | ✅ **0** — 21 de 21 documentos dentro do teto |
...
árvore de `Meus-Projetos/` inteira — não recebe a raiz do projeto como argumento
(`scripts/pre-voo.mjs:711-728` chama o script sem repassar o parâmetro) — e encontra
