...
>
> ⚠️ **Contagens atualizadas em 05/08/2026.** Este documento é vivo — não é fatia fechada de
> `docs/historico/`, e alguém o lê para reconstruir o produto. Quando **D12** entrou no catálogo
> (*"todo turno que existe pede pelo menos uma pessoa"*, achado da quinta auditoria externa), as
> três menções a "12 regras duras" viraram **12**. O portão `npm run fatos:conferir` cobra isso:
> regra nova entra no catálogo **e** no texto no mesmo passo, senão quem reconstrói pela
> documentação recria onze.
...
Busca com retrocesso, instantânea e gratuita. Garante as 12 regras duras por construção e descobre o maior piso possível. **É a rede**: sempre existe uma escala válida na mesa, independentemente do que a IA fizer.
...
Para **cada** uma das 12 regras duras e 5 de qualidade:
...
| **ERRO 20** — portão com escopo implícito | Validação que confere 8 das 12 regras duras e não avisa | Enumerar o universo de regras **primeiro**; exceção é declarada e contada |
